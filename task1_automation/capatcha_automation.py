...
# ===== PROXY ROTATION =====
PROXIES = [
    "http://xhgqsxhz:ye1i4wrkgz8u@31.59.20.176:6754",
    "http://xhgqsxhz:ye1i4wrkgz8u@23.95.150.145:6114",
    "http://xhgqsxhz:ye1i4wrkgz8u@198.23.239.134:6540",
    "http://xhgqsxhz:ye1i4wrkgz8u@45.38.107.97:6014",
    "http://xhgqsxhz:ye1i4wrkgz8u@107.172.163.27:6543",
    "http://xhgqsxhz:ye1i4wrkgz8u@198.105.121.200:6462",
    "http://xhgqsxhz:ye1i4wrkgz8u@64.137.96.74:6641",
    "http://xhgqsxhz:ye1i4wrkgz8u@216.10.27.159:6837",
    "http://xhgqsxhz:ye1i4wrkgz8u@23.26.71.145:5628",
    "http://xhgqsxhz:ye1i4wrkgz8u@23.229.19.94:8689",
]
...
TIMEZONES = [
    ...
    {'id': 'Europe/London', 'lat': 51.5074, 'lon': -0.1278},
    {'id': 'Europe/Paris', 'lat': 48.8566, 'lon': 2.3522},
    {'id': 'Europe/Berlin', 'lat': 52.5200, 'lon': 13.4050},
    {'id': 'Asia/Tokyo', 'lat': 35.6762, 'lon': 139.6503},
]
...
async def solve_with_stealth(target_url: str, proxy: str | None = None, headless: bool = False) -> dict:
    """
    ...
    async with Stealth().use_async(async_playwright()) as p:
        ...
        # Create context with randomized realistic settings
        context_options = {
            'viewport': viewport,
            ...
            'geolocation': {'latitude': timezone['lat'], 'longitude': timezone['lon']},
        ...
        if proxy:
            context_options['proxy'] = parse_proxy(proxy)
        ...
        await human_like_mouse_move(page, random.randint(200, 600), random.randint(200, 400))
        ...
        try:
            ...
            if result_text and '{' in result_text:
                ...
                result["token"] = google_response.get('token') or result_json.get('token')
                
                quality = "  ✅ GOOD >= 0.9" if result["score"] and result["score"] >= 0.9 else ""
                print(f"Score: {result['score']}{quality} | Success: {result['success']}")
...
async def run_batch_tests(num_tests: int = 250, headless: bool = False, use_proxy: bool = True):
    ...
    print(f"{'='*80}")
    print(f"Runs: {num_tests} | Successful extractions: {total} | Time: {elapsed:.1f} min")
    ...
    with open(results_file, "w", encoding="utf-8") as f:
        ...
        for idx, r in enumerate(results, 1):
            if r["score"] is not None:
                ...
                f.write(f"[{idx:3d}] Score: {r['score']:.3f} | Success: {r['success']} | Token: {token_short}\n")
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--tests-no", type=int, default=1, help="Number of tests to run (default: 1)")
    parser.add_argument("--no-proxy", action="store_true", help="Run without proxies")
    args = parser.parse_args()
    
    if args.tests_no == 1:
        # Single test mode
        asyncio.run(solve_with_stealth(TARGET_URL, headless=args.headless))
    else:
        # Batch test mode
